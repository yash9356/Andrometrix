...
public class RecyclerViewAdapter  extends RecyclerView.Adapter<RecyclerViewAdapter.MyViewHolder> implements Filterable {
    ...
    private List<DataModel> listsearch;
    ...
    public RecyclerViewAdapter(List<DataModel> list) {
        ...
        this.list =new ArrayList<>(listsearch);
    ...
    private final Filter filterlist =new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if(constraint == null || constraint.length() ==0){
                filtered_list.addAll(listsearch);
            }else {
                //uppercase or lower case will same
                String filterpattern =constraint.toString().toLowerCase().trim();
                for (DataModel dataModel : listsearch){
                    if(dataModel.getTitle().toLowerCase().contains(filterpattern)){
                        filtered_list.add(dataModel);
                    }
                }
            }
